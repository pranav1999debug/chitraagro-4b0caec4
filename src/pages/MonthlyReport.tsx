...
import { customerStore, transactionStore, paymentStore, procurementStore, expenseStore, staffStore, attendanceStore } from '@/lib/store';
...
export default function MonthlyReport() {
  ...
  const allProcurement = procurementStore.getAll();
  const allExpenses = expenseStore.getAll();
  const allStaff = staffStore.getAll();
  const allAttendance = attendanceStore.getAll();

  const data = useMemo(() => {
    const monthTx = allTx.filter(tx => tx.dateKey.startsWith(yearMonth));
    const monthPayments = allPayments.filter(p => p.dateKey.startsWith(yearMonth));
    const monthProcurement = allProcurement.filter(p => p.dateKey.startsWith(yearMonth));
    const monthExpenses = allExpenses.filter(e => e.dateKey.startsWith(yearMonth));
    const monthAttendance = allAttendance.filter(a => a.dateKey.startsWith(yearMonth));

    const totalLitersSold = monthTx.reduce((s, tx) => s + tx.quantity, 0);
    const totalSalesAmount = monthTx.reduce((s, tx) => s + tx.total, 0);
    const totalMilkPurchased = monthProcurement.reduce((s, p) => s + p.quantity, 0);
    const procurementCost = monthProcurement.reduce((s, p) => s + p.total, 0);
    const totalExpenses = monthExpenses.reduce((s, e) => s + e.amount, 0);
    const totalPaymentsReceived = monthPayments.reduce((s, p) => s + p.amount, 0);
    ...
    const totalSalary = allStaff.reduce((s, staff) => {
      const staffAtt = monthAttendance.filter(a => a.staffId === staff.id && a.present);
      const presentDays = staffAtt.length;
      const dailyRate = staff.salary / 30;
      return s + presentDays * dailyRate;
    }, 0);
    const totalAdvance = monthAttendance.reduce((s, a) => s + a.advanceAmount, 0);

    // Pending balance (all customers)
    const customers = customerStore.getAll();
    const totalPending = customers.reduce((s, c) => {
      const custTx = allTx.filter(tx => tx.customerId === c.id);
      const custPay = allPayments.filter(p => p.customerId === c.id);
      const balance = c.openingBalance + custTx.reduce((a, tx) => a + tx.total, 0) - custPay.reduce((a, p) => a + p.amount, 0);
      return s + balance;
    }, 0);
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const daySales = monthTx.filter(tx => tx.dateKey === dk).reduce((s, tx) => s + tx.total, 0);
  ...
  }, [allTx, allPayments, allProcurement, allExpenses, allStaff, allAttendance, yearMonth, daysInMonth]);
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `report_${yearMonth}.txt`;
    a.click();
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="stat-card">
          ...
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">{t('report.totalLiters', lang)}</span>
            <span className="font-number font-bold">{data.totalLitersSold} Ltrs</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">{t('report.salesAmount', lang)}</span>
            <span className="font-number font-bold">₹{data.totalSalesAmount}</span>
          </div>
        </div>
        ...
        <div className="stat-card">
          ...
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">{t('report.milkPurchased', lang)}</span>
            <span className="font-number font-bold">{data.totalMilkPurchased} Ltrs</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">{t('report.procurementCost', lang)}</span>
            <span className="font-number font-bold">₹{data.procurementCost}</span>
          </div>
        </div>
        ...
        <div className="stat-card">
          ...
          <div className="flex justify-between text-sm mb-1">
            <span className="text-muted-foreground">{t('dashboard.totalExpenses', lang)}</span>
            <span className="font-number font-bold">₹{data.totalExpenses}</span>
          </div>
          {Object.entries(data.expenseByCategory).map(([cat, amt]) => (
            <div key={cat} className="flex justify-between text-xs pl-3">
              <span className="text-muted-foreground">{cat}</span>
              <span className="font-number">₹{amt}</span>
            </div>
          ))}
        </div>
        ...
        <div className="stat-card">
          ...
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">{t('report.totalSalary', lang)}</span>
            <span className="font-number font-bold">₹{data.totalSalary}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">{t('report.totalAdvance', lang)}</span>
            <span className="font-number font-bold">₹{data.totalAdvance}</span>
          </div>
        </div>
        ...
        <div className="stat-card">
          ...
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">{t('report.paymentsReceived', lang)}</span>
            <span className="font-number font-bold text-primary">₹{data.totalPaymentsReceived}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">{t('report.pendingBalance', lang)}</span>
            <span className="font-number font-bold text-destructive">₹{data.totalPending}</span>
          </div>
        </div>
        ...
        <div className={`stat-card border-2 ${data.profitLoss >= 0 ? 'border-primary' : 'border-destructive'}`}>
          ...
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">{t('report.revenue', lang)}</span>
            <span className="font-number font-bold">₹{data.totalRevenue}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">{t('report.totalCosts', lang)}</span>
            <span className="font-number font-bold">₹{data.totalCosts}</span>
          </div>
          <div className="border-t border-border pt-2 mt-2 flex justify-between items-center">
            ...
            <span className={`font-number font-bold text-xl ${data.profitLoss >= 0 ? 'text-primary' : 'text-destructive'}`}>
